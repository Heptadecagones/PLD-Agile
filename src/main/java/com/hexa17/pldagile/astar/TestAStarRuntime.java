...
public class TestAStarRuntime {
    ...
    // Let's assume that on a road map, an average of four roads depart from each
    // intersection
    ...
    private static void runTests(int iteration, boolean warmup) {
        ...
        for (int numNodes = MIN_NODES; numNodes <= MAX_NODES; numNodes = Math.min((int) (numNodes * 1.5),
                numNodes + 25_000)) {
    ...
    private static long runTestForGraphSize(int numNodes, int numEdges) {
        ...
        while (shortestPath == null) {
            ...
            ValueGraph<NodeWithXYCoordinates, Double> graph = TestGraphFactory.createGraph(nodes, numEdges);

            NodeWithXYCoordinates source = nodes[0];
            NodeWithXYCoordinates target = nodes[1];

            // Don't measure generating the heuristic function - it needs to be generated
            // only once per
            // graph and can, for example, be included in a satnav's data.
            HeuristicForNodesWithXYCoordinates heuristic = new HeuristicForNodesWithXYCoordinates(graph, target);
