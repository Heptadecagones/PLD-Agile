...
/**
 * Implementation of the A* algorithm with a {@link PriorityQueue} and a data
 * structure holding the
 * actual node, its total distance and its predecessor
 * ({@link AStarNodeWrapper}).
 *
...
public class AStarWithPriorityQueue {
     ...
     * @param heuristic the heuristic function mapping a node to its the minimal
     *                  remaining costs
    ...
    public static <N extends Comparable<N>> List<N> findShortestPath(
        ...
        // Add source to queue
        AStarNodeWrapper<N> sourceWrapper = new AStarNodeWrapper<>(source, null, 0.0, heuristic.apply(source));
        ...
        while (!queue.isEmpty()) {
            ...
            for (N neighbor : neighbors) {
                ...
                if (neighborWrapper == null) {
                    neighborWrapper = new AStarNodeWrapper<>(
