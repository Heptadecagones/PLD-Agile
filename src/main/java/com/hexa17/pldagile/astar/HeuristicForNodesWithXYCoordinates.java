...
public class HeuristicForNodesWithXYCoordinates implements Function<NodeWithXYCoordinates, Double> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(HeuristicForNodesWithXYCoordinates.class);
    ...
    /**
     * Calculates the maximum speed possible on any path in the graph. The speed of
     * a path is
     * calculated as: euclidean distance between the path's nodes, divided by its
     * cost.
     *
     * @param graph the graph
    ...
    /**
     * Calculates the speed on a path in the graph. The speed of a path is
